...
static size_t
BakeTallyData(const uint8_t in[], size_t inLen, uint8_t out[], size_t outLen)
{
    uint8_t *outPtr = out;
...
bool
IsTallyPacket(const Packet *pkt)
{
    return pkt->header.dest == 0xFF && pkt->commandInfo.category == 0xFF &&
           pkt->commandInfo.parameter == 0xFF;
}

void
ReceiverInit(RunState *runState)
{
...
void
ReceiverWireLoop(void *_runState)
{
    ...
    while (true) {
        ...
        if (runState->queue.TPeek(&peekHeader) > 0) {
            const size_t bytesInBuffer     = dataPtr - data;
            const size_t totalPacketLength = peekHeader.GetPaddedTotalSize();
            ...
            if (totalPacketLength < NUM(data) - bytesInBuffer) {
                ...
                if (IsTallyPacket(pkt)) {
                    ...
                    const size_t tallyLength = BakeTallyData(pkt->data, pkt->header.GetDataSize(),
                                                             tallyData, NUM(tallyData));
...
ReceiverSerialLoop(void *_runState)
{
    ...
    while (true) {
        ...
        const size_t paddedSize = pkt.header.GetPaddedTotalSize();
