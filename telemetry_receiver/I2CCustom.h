...
{
public:
  I2CCustom(int wireAddress) : m_wireAddress(wireAddress)
  {
  }
...
template <typename T>
uint32_t I2CCustom<T>::regRead32(uint16_t address) const
{
  uint32_t reg;
...
template <typename T>
void I2CCustom<T>::regWrite32(uint16_t address, uint32_t value) const
{
...
template <typename T>
uint16_t I2CCustom<T>::regRead16(uint16_t address) const
{
  uint16_t reg;
...
template <typename T>
void I2CCustom<T>::regWrite16(uint16_t address, uint16_t value) const
{
  auto data = EndianSwap(value);
...
template <typename T>
uint8_t I2CCustom<T>::regRead8(uint16_t address) const
{
  uint8_t res;
...
template <typename T>
void I2CCustom<T>::regWrite8(uint16_t address, uint8_t value) const
{
  regWrite(address, &value, sizeof(uint8_t));
}

template <typename T>
void I2CCustom<T>::regRead(uint16_t address, uint8_t values[], int length) const
{
  if (length <= 0)
  {
    return;
  ...
  i2c_master_cmd_begin(I2C_NUM_0, cmd, (1000 / portTICK_RATE_MS)); // Not error checked since timeout is a valid outcome during startup
  i2c_cmd_link_delete(cmd);
}

template <typename T>
void I2CCustom<T>::regWrite(uint16_t address, const uint8_t values[], int length) const
{
  if (length <= 0)
  {
    return;
